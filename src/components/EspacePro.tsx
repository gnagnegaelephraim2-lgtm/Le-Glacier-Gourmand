...
import { Check, ClipboardList, ShoppingCart, Calendar, Phone, MapPin, Building, FileText, Send, X } from 'lucide-react';
...
export default function EspacePro() {
  ...
  const { isEspaceProOpen, closeEspacePro } = useEspacePro();
    ...
    <AnimatePresence>
      {isEspaceProOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[200] bg-cream overflow-y-auto"
        >
          {/* Sticky Header */}
          <div className="sticky top-0 z-10 bg-cream/95 backdrop-blur-md border-b border-forest/10 shadow-sm">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <Logo className="h-10 sm:h-12" showText={false} variant="dark" />
                <div className="hidden sm:block">
                  <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-gold bg-gold/10 px-3 py-1 rounded-full border border-gold/20">B2B Pro</span>
                </div>
              </div>
              <button
                onClick={closeEspacePro}
                className="p-2.5 rounded-full bg-forest/5 hover:bg-forest hover:text-cream text-forest transition-all duration-300"
                aria-label="Close"
              >
                <X size={22} />
              </button>
